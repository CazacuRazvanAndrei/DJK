...
Create a new folder called node and navigate to it:
Copy
$ mkdir node && cd node
Run npm init in this folder, and accept all defaults except the entry point, which you change from the index.js default to server.js.
We need to add express to our project with the following:
Copy
$ npm install --save express
Now we need to install the Prometheus adapter for Node Express with the following:
Copy
$ npm install --save prom-client
Add a file called server.js to the folder with this content:
Copy
...
To the preceding code, add the following snippet to initialize the Prometheus client:
Copy
const client = require("prom-client");
const register = client.register;
const collectDefaultMetrics = client.collectDefaultMetrics;
collectDefaultMetrics({ register });
Next, add an endpoint to expose the metrics:
Copy
app.get('/metrics', (req, res) => {
  res.set('Content-Type', register.contentType);
  res.end(register.metrics());
});
Now let's run this sample microservice:
Copy
$ npm start
...
Let's now try to access the metrics at the /metrics endpoint, as we defined in the code:
Copy
...
Add the following code snippet to server.js to define a custom counter called my_hello_counter:
Copy
const helloCounter = new client.Counter({ 
  name: 'my_hello_counter', 
  help: 'Counts the number of hello requests',
});
To our existing /hello endpoint, add code to increase the counter:
Copy
app.get('/hello', (req, res) => {
  helloCounter.inc();
  const { name = 'World' } = req.query;
...
Rerun the application with npm start.
To test the new counter, let's access our /hello endpoint twice:
Copy
$ curl localhost:3000/hello?name=Sue
We will get this output when accessing the /metrics endpoint:
Copy
$ curl localhost:3000/metrics
